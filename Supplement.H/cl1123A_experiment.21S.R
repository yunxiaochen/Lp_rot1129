...
gen.A<-function(){
  ...
  diag(Dprime[1:3,])=0
  diag(Dprime[4:6,])=0
  Aprime = rbind(X%*%solve(Qt),Dprime)
  A  = rbind(X, Dprime %*% Qt)
  return(A)}
#15*3small loading##########################################
L=gen.A()
...
P<-c(0.24, 0.32, 0.45, 0.65, 0.18, 0.64, 0.67, 0.51, 0.49, 0.06, 0.19, 0.16, 0.52, 0.33, 0.57,0.40,0.54,0.69,0.32,0.58,0.66)
#########################################################
n_row<- nrow(L)
n_col<- ncol(L)
...
t=proc.time()[3]-t1[3]
save(out,L,B,P,sim,N,seed, t, file = paste0("Lp.1123A.",nrow(L),"N", N, ".RData"))
}
stopCluster(cl)